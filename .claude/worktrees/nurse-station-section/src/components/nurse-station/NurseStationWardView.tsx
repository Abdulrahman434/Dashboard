...
function RoomCard({
  ...
  gender?: string;
  age?: string;
  onUpdateManual: (info: ManualRoomInfo) => void;
  onClearManual?: () => void;
  onClick?: () => void;
}) {
  const handleInputChange = (field: string, value: string) => {
    onUpdateManual({
      mrn: field === "mrn" ? value : mrn,
      gender: field === "gender" ? value : gender || "Female",
      doa: field === "doa" ? value : doa || "25/5/2025",
      age: "43y",
    });
  };
  ...
  return (
    <div
      ...
      className={`text-left rounded-lg overflow-hidden shadow-sm border border-gray-200 bg-white flex flex-col justify-between min-h-[175px]
                 transition hover:shadow-md hover:scale-[1.01] ${isOccupied ? "cursor-pointer select-none" : ""}`}
    >
      {/* Header */}
      <div
      ...
      <div className="p-3 flex-1 flex flex-col justify-between bg-white font-['Poppins',sans-serif]">
        ...
        <div className="flex items-center justify-between mb-1.5">
          ...
          {isOccupied ? (
            ...
            <span className="bg-[#F0FDF4] text-[#01C874] font-bold px-2 py-0.5 rounded text-[9px] tracking-wide uppercase">
              Available
            </span>
          )}
        </div>
        ...
        {isOccupied ? (
          ...
          <div className="space-y-1.5 pt-1 font-['Poppins',sans-serif]" onClick={(e) => e.stopPropagation()}>
            <div className="text-[12px] font-semibold text-[#01C874] mb-1">Available</div>
            ...
            <div className="grid grid-cols-1 gap-1">
              <input
                ...
                placeholder="Enter MRN (manually)..."
                className="w-full text-[9px] px-2 py-0.5 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-[#09ADEA] font-sans"
              />
              <div className="flex gap-1">
                <select
                  value={gender || "Female"}
                  onChange={(e) => handleInputChange("gender", e.target.value)}
                  className="w-1/2 text-[9px] px-1 py-0.5 border border-gray-200 rounded bg-white focus:outline-none focus:ring-1 focus:ring-[#09ADEA] font-sans"
                >
                ...
                <input
                  ...
                  placeholder="DOA: 25/5/2025"
                  className="w-1/2 text-[9px] px-1.5 py-0.5 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-[#09ADEA] font-sans"
                />
              </div>
            </div>
          </div>
...
export default function NurseStationWardView({
  ...
  const mappedRooms = useMemo(() => {
    ...
    return activeStation.rooms.map((r) => {
      ...
      return {
        ...
        doa: manual?.doa || dev?.doa || (isOccupied ? "25/5/2025" : ""),
  ...
  }, [activeStation, devices, manualOccupancies]);
  ...
  const handleRoomClick = (room: any) => {
    ...
    nurseActions.updatePatient({
      mrn: room.mrn,
      room: room.no,
      age: room.age?.replace("y", "") || "43",
      admissionDate: room.doa || "25/5/2025",
      name: room.gender === "Male" ? "Omar Saleh" : "Sara Saleh",
      nameAr: room.gender === "Male" ? "عمر صالح" : "سارة صالح",
      bed: "1",
